...
| **Reactive fallback** | When the forecasting service is unavailable, the operator keeps using its last forecast for up to ten minutes and then falls back to its own reactive calculation from live metrics; a forecast that fails the divergence check is discarded at once. A KEDA `ScaledObject` can be kept on the same Deployment as an independent backstop; pause it while the operator is active (KEDA's `autoscaling.keda.sh/paused` annotation) so the two controllers do not compete. |
